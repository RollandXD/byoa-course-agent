...
本项目实现了一个面向“软件产品综合研发实践”实验二的交互式 BYOA 课程助手。它采用命令行终端作为交互界面，用户可以通过 `python -m byoa_agent chat` 进入类似 Claude Code / Codex CLI 的连续对话环境，也可以使用 `/tools`、`/check`、`/log`、`/report`、`/demo` 等固定命令获取稳定输出。Agent 的大模型部分使用 DeepSeek Function Calling，本地工具负责读取课程 PPT、实验报告模板、项目仓库文件和 JSONL 工具调用日志。当前项目暴露 {tool_count} 个工具，超过实验要求的至少 2 个工具，并通过自检工具把 README、prompt、source code、tests、报告草稿和运行日志等交付证据串联起来。
